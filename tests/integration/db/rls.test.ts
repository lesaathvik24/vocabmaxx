...
beforeAll(async () => {
    ...
    const a = await wordsQ.insert({ userId: USER_A, term: 'alpha', definition: 'a-def', examples: ['ex'], source: 'dictionary' })
    const b = await wordsQ.insert({ userId: USER_B, term: 'bravo', definition: 'b-def', examples: ['ex'], source: 'dictionary' })
