...
describe('srs queries + service', () => {
    it('initialize creates a default srs_state row due now', async () => {
        const w = await wordsQ.insert({ userId: USER_A, term: 'init', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null })
    ...
    it('findDue returns only rows with due_date <= asOf', async () => {
        const past = await wordsQ.insert({ userId: USER_A, term: 'past', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null })
        const future = await wordsQ.insert({ userId: USER_A, term: 'future', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null })
    ...
    it('recordReview updates state and appends a review_log in one transaction', async () => {
        const w = await wordsQ.insert({ userId: USER_A, term: 'tx', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null })
    ...
    it('recordReview returns word_not_found error when no srs row exists', async () => {
        const w = await wordsQ.insert({ userId: USER_A, term: 'orphan', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null })
