...
describe('importService.saveBulk', () => {
    ...
    it('aggregates added, skipped, failed without throwing', async () => {
        ...
        vi.mocked(deps.defFetch)
            .mockResolvedValueOnce(ok({ term: 'alpha', def: { definition: 'd', examples: ['e'], source: 'dictionary' as const, phonetic: null, audioUrl: null, senses: [] } }))
            .mockResolvedValueOnce(ok({ term: 'beta', def: { definition: 'd', examples: ['e'], source: 'dictionary' as const, phonetic: null, audioUrl: null, senses: [] } }))
            .mockResolvedValueOnce(err({ kind: 'not_found' as const }))

        vi.mocked(deps.wordSave)
            .mockResolvedValueOnce(ok({ id: '1', userId: 'u', term: 'alpha', definition: 'd', examples: ['e'], source: 'dictionary', phonetic: null, audioUrl: null, senses: null, addedAt: new Date() }))
    ...
    it('does not throw on individual save failure', async () => {
        const deps = makeSaveDeps()
        vi.mocked(deps.defFetch).mockResolvedValue(ok({ term: 'word', def: { definition: 'd', examples: ['e'], source: 'llm' as const, phonetic: null, audioUrl: null, senses: [] } }))
